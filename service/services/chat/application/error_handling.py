...
def map_to_http_exception(error: Exception) -> HTTPException:
    ...
    if isinstance(error, ConfirmationOfferError):
        messages = {
            "confirmation_expired": "Confirmation expired",
            "confirmation_forbidden": "Confirmation not found",
            "confirmation_busy": "Confirmation is already being processed",
            "confirmation_invalid": "Confirmation is invalid",
            "confirmation_unavailable": "Confirmation service unavailable",
        }
        return HTTPException(
            status_code=error.status_code,
            detail={
                "code": error.code,
                "message": messages.get(error.code, "Confirmation failed"),
            },
        )
...
def normalize_response_metadata(
    ...
    data = sanitize_mode_offer_metadata(metadata)
