...
class ProcessChatMessageHandler(ChatCommandPort):
    def __init__(
        self,
        job_service: JobOrchestrationPort,
    ...
    async def dispatch_and_wait(
        ...
        offer_id = str((command.session_data or {}).get("confirmed_offer_id") or "")
        ...
        if offer_id:
            ...
            if duplicate:
                return ChatReplyResult(
                    reply="Подтверждение уже принято, запуск выполняется.",
                    thread_id=command.thread_id,
        ...
        try:
            ...
            if offer_id:
                await release_confirmation_claim(self.redis_client, offer_id)
        ...
        if confirmation_anchor is not None:
            try:
                await mark_confirmation_accepted(
                    ...
                    confirmation_anchor,
                    job_id=str(job_response.job_id),
                    celery_task_id=str(getattr(task, "id", "") or "") or None,
                )
            except Exception:
                logger.error(
                    "confirmation state update failed",
                    extra={"component": "chat", "failure_code": "confirmation_unavailable"},
                )

        try:
            # AsyncResult.get() — блокирующий: вызванный напрямую из async-функции, он
            # замораживает ВЕСЬ event loop на timeout_sec (сериализует все запросы
            # приложения). Уводим ожидание в тред-пул, чтобы loop оставался живым.
            result = await asyncio.to_thread(task.get, timeout_sec)
        except Exception as exc:
            raise JobExecutionError("Failed waiting for chat task result") from exc

        if not isinstance(result, dict):
            raise JobExecutionError(
                f"Task returned unexpected result type: {type(result).__name__}"
            )
