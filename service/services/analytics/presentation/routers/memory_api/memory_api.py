...
@memory_router.get("/{user_id}/dashboard")
async def get_user_memory_dashboard(
    user_id: str,
    auth_profile: Annotated[AuthProfile, Depends(check_auth)],
    service: Annotated[MemoryService, Depends(get_memory_service)],
) -> dict:
    """Дашборд памяти MemOS: факты по типам (text/pref/tool/skill) + счётчики.
    Пусто ({}), если активна не memos-память."""
    effective_user_id = _ensure_owner_access(user_id, auth_profile)
    return await service.dashboard(effective_user_id)
...
async def clear_user_memory(
    ...
    """Стереть долговременную память пользователя целиком: факты + семантическую MemOS.

    Удалять факты по одному было можно, а стереть всё — нет: при десятках записей это
    не операция, а занятие. Отдаём РАЗДЕЛЬНЫЙ отчёт (`facts_deleted`, `memos`), потому
    что половины независимы: MemOS может быть выключен или лежать, и тогда фронт обязан
    сказать «факты стёрты, семантическая память — нет», а не общее «готово».

    ⚠️ Ответ 200 с отчётом, а не 204: у 204 нет тела, и сказать в нём, что именно
    стёрлось, невозможно.
    """
    effective_user_id = _ensure_owner_access(user_id, auth_profile)
    return await service.forget_everything(effective_user_id)
