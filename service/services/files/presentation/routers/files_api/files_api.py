    ...
    if file_name := file.filename:
        ...
        if not any(
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"{config.file.settings.allowed_extensions} allowed."
                ),
            )
        ...
        if len(content) > config.file.settings.max_file_size_byte:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                detail=(
                    f"File too large. Max size: {config.file.settings.max_file_size_byte} bytes."
                ),
            )
...
async def get_file(
    ...
    download = await service.get_presigned_url_by_key(file_key=meta.file_key)
    return FileDetailResponse(file_id=meta.file_id, file_url=meta.file_url, download_url=download)


@files_router.get(
    "/files/v1/{file_id}/download",
    summary="Download an owned file by its opaque identifier",
)
async def download_file(
    ...
    """Resolve storage only after the authenticated owner check.

    Assistant metadata carries the opaque ``file_id`` only.  Storage keys and
    capabilities therefore never become durable chat metadata or HTML hrefs.
    """
    ...
    if not download:
        raise HTTPException(
            ...
            detail="File is temporarily unavailable",
        )
    return RedirectResponse(url=download, status_code=status.HTTP_307_TEMPORARY_REDIRECT)
