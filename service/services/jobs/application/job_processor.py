...
class NewJobProcessor:
    ...
    async def process_new_jobs(self) -> NoReturn:
        while True:
            ...
            logger.info(
                "No new jobs found. Waiting before next check: "
                f"{self.config.settings.processing_interval_sec} seconds."
            )
    ...
    async def _save_job_result(self, job: JobLogic) -> JobLogic:
        try:
            ...
            raise RuntimeError("Failed to save job result") from err
