...
async def jobs_ws(
    ...
    try:
        ...
        if requester_id and requester_id != _ANONYMOUS_USER_ID:
            owned = True
            try:
                job = await job_service.repository.fetch_job_by_id(UUID(job_id), UUID(requester_id))
                owned = job is not None
            except Exception:
                logger.exception("jobs_ws: ownership check failed for job=%s", job_id)
            if not owned:
                await websocket.accept()
                await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
                return
