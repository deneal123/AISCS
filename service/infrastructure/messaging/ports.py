...
from service.infrastructure.agents_client.ports import StreamPort
...
class CeleryJobQueuePort(JobQueuePort):
    ...
    def cancel_task(self, task_id: str) -> bool:
        # terminate=False: НЕ хард-килл. SIGTERM убивал бы prefork-воркер до его
        # except/finally — резерв кредитов не освобождался, статус/ход не обновлялись.
        # Кооперативная остановка идёт через Redis-флаг chat:cancel:{task_id}, который
        # воркер проверяет в стрим-цикле и сворачивается штатно (release + FAILURE +
        # персист частичного ответа); revoke(terminate=False) не даёт задаче
        # переисполниться, но не рвёт уже идущую насильно.
        result = AsyncResult(task_id, app=celery_app)
