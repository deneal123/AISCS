...
def _is_async(client) -> bool:
    """Асинхронный ли клиент. 🔴 Правило ОДНО на сервис — см. `redis_manager.is_async_client`.

    Здесь оно жило семью копиями, и все семь судили ПО КОМАНДЕ: `iscoroutinefunction(xtrim)`
    на `redis.asyncio` = ЛОЖЬ, поэтому асинхронный клиент уходил в `to_thread` и получал НЕ
    ДОЖДАННУЮ корутину. Команда не исполнялась ВОВСЕ и молчала об этом.
    """
    from service.infrastructure.cache.redis_manager import is_async_client

    return is_async_client(client)
...
async def last_entry_id(redis_client, stream: str) -> str:
    """Идентификатор ПОСЛЕДНЕЙ записи потока прямо сейчас (или «0-0» на пустом).

    🔴 Нужен, чтобы курсор «читать только новое» брался ЗАРАНЕЕ, а не при первом
    блокирующем чтении. Живой замер: `XREAD` с «$» фиксирует курсор в момент вызова, а
    вызов происходил уже после того, как сокет начал принимать сообщения, — самые первые
    события прогона (`auto_decision`, `mode_offer`) публиковались в зазор и терялись
    НАВСЕГДА. Снаружи это выглядело как «трейс без первых шагов» и «предложение режима
    не пришло».
    """
    try:
        info = await get_stream_info(redis_client, stream)
        last = info.get("last_entry")
        if isinstance(last, list | tuple) and last:
            candidate = last[0]
        elif isinstance(last, dict):
            candidate = next(iter(last), None)
        else:
            candidate = None
        if candidate:
            return candidate.decode() if isinstance(candidate, bytes) else str(candidate)
    except Exception:
        logger.debug("last_entry_id: не удалось прочитать хвост потока %s", stream, exc_info=True)
    # ⚠️ ФОЛБЭК — «$», а НЕ «0-0». Не смогли узнать хвост — деградируем до прежнего
    # поведения (потерять первые события), а не до чтения потока С НАЧАЛА: на треде с
    # историей это переиграло бы клиенту тысячи старых чанков. Пустой поток «$» и «0-0»
    # трактуют одинаково, так что новый диалог от фолбэка не страдает.
    return "$"
