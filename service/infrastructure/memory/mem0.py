...
# Дистрибутив называется `mem0ai`, а ИМПОРТИРУЕМЫЙ пакет — `mem0`. Здесь был второй
# фолбэк `from mem0ai import ...`, который не мог сработать никогда: модуля с таким
# именем не существует ни при какой установке. Вложенный мёртвый except маскировал
# это тем, что выглядел как забота о необязательной зависимости.
try:
    from mem0 import MemoryClient as _Mem0MemoryClient  # type: ignore[import-not-found]
except Exception:  # pragma: no cover - зависимость необязательна
