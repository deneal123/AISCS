    ...
    def validate_http_jwt(self, auth_token: str | None) -> AuthProfile:
        ...
        try:
            ...
            try:
                ...
                # Подпись валидна, но claims не проходят схему (напр. чужой/старый
                # `type`). Это НЕ ошибка сервера: раньше pydantic-исключение
                # улетало наружу как 500 на КАЖДОМ authed-эндпоинте.
                raise jwt.InvalidTokenError("Invalid claims in token") from exc
