...
def get_billing_service(request: HTTPConnection) -> BillingService:
    return get_app_container(request).services.billing_service


def get_admin_service(request: HTTPConnection) -> Any:
    return get_app_container(request).services.admin_service


def get_optional_redis_client(request: HTTPConnection) -> Any:
    # OPTIONAL: контейнер может быть не инициализирован (изолированный тест без
    # lifespan) — тогда redis просто недоступен (None), а не 500. Вызывающий код
    # обязан переносить None (это и подразумевает имя get_optional_*).
    try:
