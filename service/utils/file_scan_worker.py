...
async def process_scan_once(redis_client: Any, scanner: Any) -> int:
    ...
    try:
        ...
        # Prefer using our stream helper when available; fall back to direct client xadd as last
        # resort.
        try:
            try:
            ...
            # best-effort: try direct xadd if available (some clients are sync) only if we didn't
            # publish
            if not published:
                try:
