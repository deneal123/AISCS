...
class DocumentSidecar:
    ...
    def new_publication(
        ...
        *,
        ...
        if not re.fullmatch(r"PUB-\d{3}", document_id):
            raise SidecarError("publication ID must match PUB-NNN")
        if self.get_document(document_id):
            raise SidecarError("publication ID already exists")
        slug = _safe_segment(slug)
        if not title.strip() or kind not in {"conference", "paper"}:
            raise SidecarError("title and kind=conference|paper are required")
        if language not in {"ru", "en"}:
            raise SidecarError("language must be ru or en")
        relative = Path("documents") / document_id
        destination = self.root / relative
        if destination.exists():
            raise SidecarError("publication source directory already exists")
        item = {
            "id": document_id,
            "slug": slug,
            "title": title.strip(),
            ...
            "authors": [],
            "deadline": None,
            "source_dir": relative.as_posix(),
            "entrypoint": (relative / "main.tex").as_posix(),
            "primary_output": None,
            "research_snapshot": "data/research-snapshot/snapshot.json",
            "evidence_refs": [],
            "approvals": [],
            "immutable": False,
            "immutable_files": [],
        ...
        if not apply:
            return {"ok": True, "applied": False, "document": item}
        self.snapshot("pre-new-publication")
        shutil.copytree(self.root / "documents" / "_templates" / "article", destination)
        registry = self.registry
        registry["documents"].append(item)
        registry["meta"]["updated_at"] = datetime.now(UTC).date().isoformat()
        atomic_write_json(self.registry_path, registry)
        checked = self.validate()
        if not checked["ok"]:
            raise SidecarError("post-create integrity failed: " + "; ".join(checked["errors"]))
        return {"ok": True, "applied": True, "document": item}
