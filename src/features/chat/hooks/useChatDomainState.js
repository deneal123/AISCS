...
export function useChatDomainState() {
  ...
  const clearThread = useCallback(() => dispatch({ type: CHAT_ACTIONS.CLEAR_THREAD }), [dispatch]);
  const replaceMessages = useCallback((items) => {
    dispatch({ type: CHAT_ACTIONS.REPLACE_MESSAGES, payload: items });
  }, [dispatch]);
  const loadHistory = useCallback((items) => {
    dispatch({ type: CHAT_ACTIONS.LOAD_HISTORY, payload: items });
  }, [dispatch]);
  ...
  const domainState = useMemo(() => ({
    ...
    error: state.error,
  }), [state.currentJob, state.error, state.loading, state.messages]);

  const actions = useMemo(() => ({ setLoading, setError, clearError, setCurrentJob, clearCurrentJob, addMessage, clearMessages, clearThread, replaceMessages, loadHistory, updateLastAgentChunk, completeLastAgentMessage, finalizeStreamWithContent }), [addMessage, clearCurrentJob, clearError, clearMessages, clearThread, completeLastAgentMessage, finalizeStreamWithContent, loadHistory, replaceMessages, setCurrentJob, setError, setLoading, updateLastAgentChunk]);
