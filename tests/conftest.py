...
def _reset_circuit_breaker_state():
    """Изоляция тестов от process-global счётчиков rate-limiter'а."""
    from service.shared.security import rate_limiter as rl

    def _clear():
        # Circuit breaker провайдеров переехал в сайдкар вместе с доменом — его
        # изоляция теперь в agents/tests/conftest.py. Здесь остался только
        # rate-limiter: он process-global, и счётчики перетекали между тестами
        # (напр. identity "u1" уже за лимитом в соседнем тесте).
