...
from service.infrastructure.agents_client.ports import AgentExecutionPort, StreamPort
...
class FakeStorage:
    ...
    async def upload_file_stream(self, *, file_key: str, stream, length: int) -> str:
...
class FakeProfileRepository:
    ...
    async def create_user(
        ...
        **kwargs,  # фейк не должен падать на каждом новом параметре согласия
    ):
    ...
    async def update_email_preferences(
        ...
        return {"id": user_id, "marketing": marketing, "service": service}
...
def test_file_storage_port_contract() -> None:
    ...
    assert (
        asyncio.run(storage.upload_file_stream(file_key=key, stream=BytesIO(b"xy"), length=2))
        == "mem://uploads/CHAT/a.txt"
    )
