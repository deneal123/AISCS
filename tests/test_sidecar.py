...
def test_document_folder_builds_after_independent_copy(tmp_path: Path) -> None:
    if shutil.which("xelatex") is None:
        pytest.skip("xelatex is unavailable")
    portable = tmp_path / "dissertation-document"
    ...
    result = subprocess.run(
        [
            ...
            r"\def\ASPADraft{1}\input{dissertation.tex}",
        ],
        cwd=portable,
        capture_output=True,
        text=True,
    ...
    assert (output / "dissertation.pdf").is_file()
