...
class _FakeMemoryService:
    ...
    async def forget_everything(self, user_id: str) -> dict:
        self.wiped.append(user_id)
        return {"facts_deleted": 34, "memos": {"ok": True, "neo4j_deleted": 12}}
...
def test_clear_user_memory_returns_a_separate_report():
    """Отчёт РАЗДЕЛЬНЫЙ, а не 204: у пустого тела не спросишь, что именно стёрлось.

    Половины независимы — MemOS может быть выключен или лежать, и тогда фронт обязан
    сказать «факты стёрты, семантическая память нет», а не общее «готово».
    """
    user_id = "11111111-1111-1111-1111-111111111111"
    fake_service = _FakeMemoryService()
    client = _make_client(fake_service, user_id)

    resp = client.delete(f"/api/memory/{user_id}")

    assert resp.status_code == 200, resp.text
    assert resp.json() == {"facts_deleted": 34, "memos": {"ok": True, "neo4j_deleted": 12}}
    assert fake_service.wiped == [user_id]
...
def test_clearing_another_users_memory_is_forbidden():
    """🔴 Стирание чужой памяти — не неточность, а разрушение чужих данных."""
    auth_user_id = "11111111-1111-1111-1111-111111111111"
    another_user_id = "22222222-2222-2222-2222-222222222222"
    fake_service = _FakeMemoryService()
    client = _make_client(fake_service, auth_user_id)

    resp = client.delete(f"/api/memory/{another_user_id}")

    assert resp.status_code == 403, resp.text
    assert fake_service.wiped == [], "чужая память всё-таки была стёрта"
