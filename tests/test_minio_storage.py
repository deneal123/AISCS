...
class TestMinioFileStorageInit:
    ...
    def test_init_skips_bucket_creation_if_exists(self, mock_minio_config):
        ...
        with patch("minio.Minio") as mock_minio_class:
            ...
            http_client = mock_minio_class.call_args.kwargs["http_client"]
...
class TestMinioFileStoragePresignedUrls:
    ...
    @pytest.mark.asyncio
    async def test_presigned_diagnostics_do_not_log_url_or_storage_key(self, minio_storage):
        storage_key_marker = "private/storage-key-marker.txt"
        capability_marker = "https://storage.example/object?X-Amz-Signature=secret-marker"
        minio_storage._client.presigned_get_object.return_value = capability_marker

        with patch("service.infrastructure.storage.minio_file_storage.logger.debug") as debug_log:
            result = await minio_storage.get_presigned_url(file_key=storage_key_marker)

        assert result == capability_marker
        template, *args = debug_log.call_args.args
        rendered = template % tuple(args)
        assert storage_key_marker not in rendered
        assert capability_marker not in rendered
        assert "MinIO download capability created" in rendered
