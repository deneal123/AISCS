...
def test_chat_ws_guest_skips_pending_and_starts_from_latest(monkeypatch):
    ...
    try:
        ...
        # ⚠️ ПРАВИЛО, А НЕ СПОСОБ: гость читает с ТЕКУЩЕГО ХВОСТА, а не с начала потока.
        # Прежняя проверка требовала буквального «$» — то есть кодировала механизм. Из-за
        # неё курсор нельзя было разрешить ЗАРАНЕЕ, а именно поздний разрешённый «$» и
        # терял первые события прогона (`auto_decision`, `mode_offer`) — найдено живым
        # прогоном. Значимо здесь одно: история треда не переигрывается.
        cursors = [next(iter(stream.values())) for stream in fake_redis.xread_calls]
        assert cursors, "консьюмер не читал поток вовсе"
        assert all(c != "0" and c != "0-0" for c in cursors), (
            f"гость читает поток с начала — клиенту переиграется вся история: {cursors}"
        )
    finally:
        app.dependency_overrides.pop(get_optional_redis_session_store, None)
        app.dependency_overrides.pop(get_optional_redis_client, None)
...
def test_jobs_ws_session_and_stream(monkeypatch):
    ...
    class FakeJobRepository:
        async def fetch_job_by_id(self, job_id, user_id):
            return None
