...
def test_create_app_fails_fast_without_cors_origins_in_prod(
    ...
    # Согласованный prod-конфиг: dev_mode=False (в dev-контейнере AUTH__DEV_MODE=true, и без
    # сброса сработал бы guard расхождения флагов, а тест проверяет ИМЕННО CORS).
    monkeypatch.setattr(service_main.config.auth, "dev_mode", False)
    monkeypatch.setattr(service_main.config.cors, "allow_origins", [])
