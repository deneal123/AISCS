...
def test_chat_upload_rejected_when_no_credits():
    # Гейт: нулевой баланс не должен получать платную транскрипцию/медиа-анализ
    # бесплатно (списание пост-фактум зажато GREATEST(0,...)). Ожидаем 402.
    from service.composition.state import get_billing_service, get_chat_application_service

    class _NoCreditsBilling:
        async def has_sufficient_credits(self, user_id):
            return False

    app.dependency_overrides[check_auth] = lambda: _fake_auth()
    app.dependency_overrides[get_billing_service] = lambda: _NoCreditsBilling()
    app.dependency_overrides[get_chat_application_service] = lambda: object()

    try:
        client = TestClient(app)
        files = {"file": ("voice.mp3", io.BytesIO(b"AUDIODATA"), "audio/mpeg")}
        resp = client.post("/api/chats/upload", files=files, data={"transcription_mode": "local"})
        assert resp.status_code == 402
        assert "кредит" in resp.text.lower()
    finally:
        app.dependency_overrides.pop(check_auth, None)
