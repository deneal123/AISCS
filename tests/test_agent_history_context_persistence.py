...
async def test_restore_pseudo_history_maps_agent_role_from_db_rows() -> None:
    ...
    # Четвёртая колонка — `metadata`: из неё в реплику дописываются прочитанные ссылки,
    # иначе следующий ход не знает, откуда взялся прошлый ответ.
    rows = [
        ("agent", "Помню ваше имя", now, {"sources": [{"url": "https://hh.test/v/1"}]}),
        ("user", "Как меня зовут?", now, None),
    ]
    ...
    # 🔴 Ссылка прошлого хода доезжает до истории: без неё ассистент на следующий вопрос
    # отвечает, что источника не видит, и просит прислать его заново.
    assistant = next(i for i in pseudo.items if i["role"] == "assistant")
    assert "https://hh.test/v/1" in assistant["content"]
