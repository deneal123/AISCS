...
@pytest.mark.asyncio
async def test_brave_anchor_fallback_fires_when_blocks_yield_nothing_usable(monkeypatch):
    """⚠️ Запасной разбор Brave включается и тогда, когда блоки НАШЛИСЬ, но отсеялись.

    Тонкость, которую легко потерять при переносе парсеров наружу: блок размечен, ссылка
    в нём есть, но она внутренняя (сам поисковик) — накопитель её отвергает, полезного
    результата нет. Условие «блоков не нашлось» такой случай пропустило бы, и Brave
    возвращал бы пустоту там, где раньше доставал ссылки якорями.
    """
    ws = importlib.import_module("service.domain.tools.web_search")
    ...
    <html><body>
      <div data-type="web">
        <a href="https://search.brave.com/search?q=next"><div class="title">Ещё результаты</div></a>
      </div>
      <a href="https://example.io/deep-dive">Разбор темы во всех подробностях</a>
    </body></html>
    """
    ...
    results = await ws.web_search("brave anchors", num_results=5)

    assert [r["url"] for r in results] == ["https://example.io/deep-dive"]
