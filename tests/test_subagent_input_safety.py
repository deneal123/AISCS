...
async def test_evaluate_input_safety_blocks_forbidden_topic() -> None:
    ...
    # P0.4: блок помечается guardrail_block, чтобы ре-роут его не обошёл.
    assert res["meta"]["guardrail_block"] is True


@pytest.mark.asyncio
async def test_evaluate_input_safety_safe_input_not_flagged_as_block() -> None:
    agent = _DummySubAgent()

    res = await agent.evaluate_input_safety("помоги решить задачу по математике")

    assert res["blocked"] is False
    assert res["meta"]["guardrail_block"] is False
