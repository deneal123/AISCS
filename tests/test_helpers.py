...
class FakeDBSession:
    ...
    def __init__(self, first_map=None, all_map=None):
        ...
        self.executed = []
        # Сырые объекты SQL (до str()) — чтобы тесты могли проверить, что в execute
        # передан именно TextClause/Executable, а не голая строка (иначе реальный
        # SQLAlchemy 2.x падает ObjectNotExecutableError, а фейк это маскировал).
        self.executed_raw = []
