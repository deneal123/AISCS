  ...
  },
  {
    ...
    languageOptions: {
      ...
      globals: {
        ...globals.browser,
        ...globals.node,
        // ⚠️ Стандартный глобал ES, которого нет в наборе `globals.browser` этой версии.
        // Без него `no-undef` краснеет на форматировании чисел и дат — то есть на
        // работающем коде, и правило пришлось бы выключить обратно.
        Intl: "readonly",
        __DEV__: true,
      },
      parserOptions: {
    ...
    rules: {
      // 🔴 ОБРАЩЕНИЕ К НЕСУЩЕСТВУЮЩЕЙ ПЕРЕМЕННОЙ — ОШИБКА, А НЕ СТИЛЬ. Плоский конфиг не
      // включает базовые правила сам, поэтому `no-undef` не действовал НИ РАЗУ: страница
      // чата падала в браузере с `ReferenceError`, а `npx eslint` отвечал «чисто».
      // Проверено мутацией: без этой строки ссылка на выдуманное имя проходит линтер.
      "no-undef": "error",
      // ⚠️ `no-unused-vars` НЕ включаем заодно: он даёт 1178 предупреждений на нынешнем
      // коде и утопил бы в них те самые ошибки, ради которых правило и заводится. Это
      // отдельное решение и отдельная уборка.
